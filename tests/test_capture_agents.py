...
from worker import capture_agent as pc_agent
...
class TestPcAgent(unittest.TestCase):
    ...
    def test_detect_window_title_prefers_windows(self, mock_x11, mock_windows):
        mock_windows.return_value = "Game Window"
        mock_x11.return_value = ""

        title, detector = pc_agent.detect_window_title()

        self.assertEqual(title, "Game Window")
        self.assertEqual(detector, "win32gui")

    @patch("worker.capture_agent.subprocess.run")
    def test_detect_window_title_x11_path(self, mock_run):
        mock_run.side_effect = [
            SimpleNamespace(returncode=0, stdout="_NET_ACTIVE_WINDOW(WINDOW): window id # 0x4600007\n"),
            SimpleNamespace(returncode=0, stdout='_NET_WM_NAME(UTF8_STRING) = "Portal 2"\n'),
        ]

        title = pc_agent._detect_window_title_x11()
        self.assertEqual(title, "Portal 2")

    def test_detect_active_game(self):
        self.assertEqual(pc_agent.detect_active_game("Playing Elden Ring now"), "Elden Ring")
